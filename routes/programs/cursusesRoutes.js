...
const cursusSchema = require("../../models/cursusModel");

const resource = 'Cursus';
...
router.post('/create', async(req, res) => {
    ...
    baseController.createDocument(req.body, resource, ['_id','__v'], cursusSchema).then(resp => {
...
router.get('/read', async(req, res) => {
    ...
    baseController.readDocuments(req.body, resource, ['description','coursesNames','year','_id','__v']).then(resp => {
        if(resp.statusMessage){ res.statusMessage = resp.statusMessage }
        if(resp.data){(res.write(JSON.stringify(resp.data)))}
        res.status(Number(resp.status))
        res.send();
...
router.post('/delete', async(req, res) => {
    ...
    baseController.deleteDocument(req.body, resource, ['description','coursesNames','year','_id','__v']).then(resp => {
        if(resp.statusMessage){ res.statusMessage = resp.statusMessage }
        if(resp.data){(res.write(JSON.stringify(resp.data)))}
        res.status(Number(resp.status))
        res.send();
